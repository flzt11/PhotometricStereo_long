...
def load_all(ps_instance, mask_filename, light_filename, data_foldername):
    ps_instance.load_mask(filename=mask_filename)
    ps_instance.load_lightnpy(filename=light_filename)
    ps_instance.load_npyimages(foldername=data_foldername)

def solve_and_save(ps_instance, normal_map_filename):
    start = time.time()
    ps_instance.solve()  # Compute
